...
int study_results(Persona population_tot[N1+N2], int matriz_adyacencia[N1+N2][N1+N2]);//compara payoff 2 vecinos
...
void study_results(int matriz_adyacencia[N1+N2][N1+N2], Persona population_tot[N1+N2]){
    ...
    compare_with_neighbour(population_tot, matriz_adyacencia);
...
double maximo_payoff(Persona population_tot[N1+N2]){
    ...
    max=min=population_tot[0].payoff;
...
int study_results(Persona population_tot[N1+N2], int matriz_adyacencia[N1+N2][N1+N2]){
    Persona aux[N1+N2];
    copy_chain(population_tot, aux,(N1+N2));

    for(int i=0;i<(N1+N2);i++){
        for(int j=0;j<(N1+N2);j++){
            if(matriz_adyacencia[i][j]){
                if(aux[i].payoff<aux[j].payoff){
                    if((rand()/((double)RAND_MAX+1))<(aux[j].payoff-aux[i].payoff)/maximo_payoff(aux))
                        population_tot[i].cooperate=aux[j].cooperate;
