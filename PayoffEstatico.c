...
#define poblaciones_parametro 1
...
int main(){
    ...
    for(int i = 0; i < 250; i++){
        ...
        double poblaciones_estudiadas_1[poblaciones_parametro];
        double poblaciones_estudiadas_2[poblaciones_parametro];
        //if (i%10 == 0)
        printf("%d\n", i);
        ...
        for (int j = 0; j < poblaciones_parametro; j++){
            ...
            */
            ...
            double fraccion_colaboradores_1 = porcentaje_colaboradores_1(population_tot);
            double fraccion_colaboradores_2 = porcentaje_colaboradores_2(population_tot);
            double fraccion_colaboradores = porcentaje_colaboradores(population_tot);
            poblaciones_estudiadas[j] = fraccion_colaboradores;
            poblaciones_estudiadas_1[j] = fraccion_colaboradores;
            poblaciones_estudiadas_2[j] = fraccion_colaboradores;
        }
        ...
        double promedio_1 = average(poblaciones_estudiadas_1, poblaciones_parametro);
        double promedio_2 = average(poblaciones_estudiadas_2, poblaciones_parametro);
        //printf("%lf \n", promedio);
        fprintf(fp, "%lf \t %lf \t %lf\t %lf \n", PD_T, promedio, promedio_1, promedio_2);
...
void study_results(int matriz_vecinos[N1+N2][N1+N2], Persona population_tot[N1+N2]){
    ...
    //max payoff es un traidor rodeado de cooperadores, es decir juega <k> veces y gana el pago del traidor en todas
    double max_diff_payoff_1 = conectividad_promedio * SD_T;//halla la maxima diferencia de payoff
    double max_diff_payoff_2 = conectividad_promedio * SD_T;//halla la maxima diferencia de payoff
...
void neighbours(int neighbours_optimized[N1+N2][N1+N2],int matriz_adyacencia[N1+N2][N1+N2]){
    ...
    for (int person = 0; person < N1+N2; person++){
        ...
        for (int i = amount_neighbours; i < N1+N2; i++)
